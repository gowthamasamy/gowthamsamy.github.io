...
function populateParentDropdowns() {
  ...
  trees[currentTree].forEach(p => {
    ...
    optF.value = p.id; optF.textContent = p.name + " (" + p.gender + ")";
    fatherSelect.appendChild(optF);
    let optM = document.createElement("option");
    optM.value = p.id; optM.textContent = p.name + " (" + p.gender + ")";
...
function savePersonData(id, name, gender, dob, death, marriage, kulamFather, kulamHusband, fatherId, motherId, imgData) {
  ...
  // Kulam logic
  if (gender.toLowerCase() === "male") kulamHusband = "";
  if (gender.toLowerCase() === "female" && !marriage) kulamHusband = "";
...
function deleteProfilePic() {
  if (!currentEditingId) return alert("Open a person first");
  let p = trees[currentTree].find(x => x.id == currentEditingId);
  if (p) {
    p.image = null;
    saveTrees();
    render();
...
function renderListView() {
  ...
  trees[currentTree].forEach(p => {
    ...
    if (p.gender.toLowerCase() === "female" && p.kulamHusband) kulamDisplay += " | Husband's: " + p.kulamHusband;
    ...
    card.innerHTML = `
      ...
      <div class="kulam">Kulam: ${kulamDisplay}</div>
...
function buildTreeBranch(person) {
  ...
  let statusClass = person.death ? "dead" : "alive";
  let kulamDisplay = person.kulamFather || "";
  if (person.gender.toLowerCase() === "female" && person.kulamHusband) {
    kulamDisplay += " | Husband's: " + person.kulamHusband;
  }

  branch.innerHTML = `
    ${person.image ? '<img src="'+person.image+'">' : ''}
    <div class="person">
      <div class="name">${person.name} (${person.gender})</div>
      <div class="kulam">Kulam: ${kulamDisplay}</div>
      <div class="status ${statusClass}"></div>
      <button onclick="openPersonModal(${person.id})">Edit</button>
      <button onclick="deletePerson(${person.id})">Delete</button>
      <button onclick="openRelationModal(${person.id})">+ Relation</button>
    </div>
  `;
...
function openRelationModal(personId) {
  ...
  let select = document.getElementById("existingPersonSelect");
...
function addRelationFromDropdown() {
  let otherId = document.getElementById("existingPersonSelect").value;
  let rel = document.getElementById("relationType").value;
  if (!otherId || !rel) return alert("Select person and relation");
  linkRelationship(currentRelationPerson, parseInt(otherId), rel);
...
function linkRelationship(id1, id2, relation) {
  ...
  switch (relation) {
    ...
    case "daughter":
      ...
      if (p1.gender.toLowerCase() === "male") p2.fatherId = p1.id;
      if (p1.gender.toLowerCase() === "female") p2.motherId = p1.id;
      break;
    case "spouse":
    ...
    case "mother":
      ...
      break;
    case "half-sibling":
      if (p1.fatherId) p2.fatherId = p1.fatherId;
      else if (p1.motherId) p2.motherId = p1.motherId;
      break;
  }
}
...
function exportAllPDF() {
  ...
  Object.keys(trees).forEach(treeName => {
    ...
    trees[treeName].forEach((p, i) => {
      ...
      let kulamDisplay = p.kulamFather || "";
      if (p.gender.toLowerCase() === "female" && p.kulamHusband) kulamDisplay += " | Husband's: " + p.kulamHusband;
      doc.text(`${i+1}. ${p.name} (${p.gender}) | Kulam: ${kulamDisplay} | DOB: ${p.dob || ""}`, 10, y);
      y += 8;
